<?php
/*
Plugin Name: Simple LDAP Login [with user creation]
Plugin URI: http://th22.info
Description:  Authenticate WordPress against LDAP [with user creation].
Version: 1.5.6
Author: Clif Griffin Development Inc & Chernousov Anton.
Author URI: http://th22.info
*/
...
class SimpleLDAPLogin {
	...
	function activate () {
		...
		$this->add_setting('admin_cn', "CN=admin,DC=mydomain,DC=org");
		$this->add_setting('admin_password', "xxxxxxxx");
	...
	function authenticate ($user, $username, $password) {
		...
		// If previous authentication succeeded, create or update LDAP user
		$user = get_user_by('login', $username);
		if ( $user ) {
			$auth = wp_authenticate_username_password($user, $username, $password);
			if ( ! is_wp_error($auth)) {
				$this->ldap_create_update($username, $password, $this->get_setting('directory') );
			return $user;}
		}
	...
	function ldap_create_update( $username, $password, $directory ) {
		$result = true;
		if ( $directory != "ad" ) {
			$this->ldap = ldap_connect( join(' ', (array)$this->get_setting('domain_controllers')), (int)$this->get_setting('ldap_port') );
			ldap_set_option($this->ldap, LDAP_OPT_PROTOCOL_VERSION, (int)$this->get_setting('ldap_version'));
			...
			$ldapbind = @ldap_bind($this->ldap, $this->get_setting('admin_cn') , $this->get_setting('admin_password'));
			if (( $ldapbind ) and ( $username != 'admin' )) {
				...
				// Create user
				$r = ldap_add($this->ldap, "cn=".$username.",".$this->get_setting('base_dn'), $info);
				// Modify password
				$userpassword = "{SHA}" . base64_encode( pack( "H*", sha1( $password ) ) );
				$info["userPassword"] = $userpassword;
				$r = ldap_mod_replace ( $this->ldap , "cn=".$username.",".$this->get_setting('base_dn') , $info );
				ldap_close($this->ldap);
				//file_put_contents('/tmp/user_auth.txt','LDAP ADMIN CONNECT OK');
			}
		}
		return apply_filters($this->prefix . 'ldap_auth', $result);
	}
